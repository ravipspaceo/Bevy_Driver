...
- (void)layoutSubviews {
    ...
    _titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:floorf(self.fontSize/2.0f)];
    _titleLabel.tag = 300;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    //Fill Color in Anti Clock Wise direction
    [timerElementPath addArcWithCenter:self.timerCenter radius:self.radius startAngle:startAngle endAngle:startAngle+0.01 clockwise: NO];
    
    //Fill Color in Clock Wise direction
    ...
    if (self.type >= DDHTimerTypeNumberOfTypes) {
        NSLog(@"The given type is not supported");
    }
    ...
    NSDictionary *labelFontAttributes = @{NSFontAttributeName: [UIFont fontWithName: @"OpenSans-Light" size:self.fontSize], NSForegroundColorAttributeName:[UIColor colorWithRed:252.0/255.0f green:125.0/255.0f blue:128.0/255.0f alpha:1.0], NSParagraphStyleAttributeName: labelStyle};
