...
//
//  Created by CompanyName on 4/23/13.
...
- (void)layoutSubviews {
    ...
    [self.indicatorView setCenter:CGPointMake(CGRectGetWidth(self.bounds)/2, CGRectGetHeight(self.bounds)/2)];
    ...
    if (self.indicatorView.hidden) {
        ...
        [self.progressView setCenter:CGPointMake(CGRectGetWidth(self.bounds)/2, CGRectGetHeight(self.bounds) - 20)];
    }
...
- (void)setImage:(UIImage *)image {
    ...
    UIImage *imageWithCrop;
    if (self.isCropRequired) {
        imageWithCrop = [self imageByScalingAndCroppingForSize:CGSizeMake(self.bounds.size.width*[UIScreen mainScreen].scale, self.bounds.size.height*[UIScreen mainScreen].scale) andImage:image];
    }
    else{
        imageWithCrop = image;
    }
...
- (void)setImage:(UIImage *)image fromURL:(NSURL *)url {
    ...
    if(self.shouldBlurr){
        ...
        [layer setRasterizationScale:0.15];
        layer.allowsEdgeAntialiasing = NO;
        [layer setShouldRasterize:YES];
    }
    //        super.image = [image blurredImage:0.1f];
    [super setImage:imageWithCrop];
    [self didChangeValueForKey:@"image"];
...
- (void)setImageWithContentsOfURL:(NSURL *)URL placeholderImage:(UIImage *)placeholderImage {
    if (URL && ![URL.absoluteString isEqualToString:self.imageContentURL.absoluteString])
    {
        ...
        if (!URL) {
            ...
            //            if(self.shouldBlurr)
            //                self.image = [self.placeholderImage blurredImage:0.4];
            [self.indicatorView stopAnimating];
...
- (void)imageDownloadFailed:(NSNotification *)notification {
    ...
    if ([url.absoluteString isEqualToString:self.imageContentURL.absoluteString]) {
        if (self.shouldHideErrorMessage) {
            ...
            [self.messageLabel setText:NSLocalizedString(@"", nil)];//Image cannot be downloaded. Tap to reload.
...
- (void)showPlaceholderImage {
    ...
    //    if(self.shouldBlurr)
    //        super.image = [self.placeholderImage blurredImage:0.4];
...
- (UIImage *)processImage:(UIImage *)image key:(NSString *)key urlString:(NSString *)url{
    ...
    if (!processedImage)
    {
        if (image) {
            ...
            /*imageCroppedAndScaledToSize:self.bounds.size
             contentMode:self.contentMode
             padToFit:NO];
             */
            //            processedImage = [self imageByScalingAndCroppingForSize:self.bounds.size andImage:processedImage];
        } else {
